...
public class UserService {
    ...
    @GET
    @Path("saveuser/{name}-{email}-{password}-{DOB}")
    @Consumes(MediaType.APPLICATION_XML)
    public Response saveUser(@PathParam("name") String name, @PathParam("email") String email, @PathParam("password") String password, @PathParam("DOB") String dob) throws JAXBException, FileNotFoundException {
        UserDAO userDAO = getUserDAO();
        Users users = userDAO.getUsers();
